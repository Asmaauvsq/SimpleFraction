...
En terme de *commits*, quelle différence constatez-vous entre cocher une (ou plusieurs) des cases *Initialize this repository with* et n'en cocher aucune ?
    > Alors dans github > create new repository si on coche une case d'initialisation (ex "Add a README file"), la forge crée automatiquement un premier commit contenant ce fichier, et le dépôt distant n'est donc pas vide dès sa création. Et si on ne coche rien, le dépôt distant est complètement vide, sans aucun commit , il faut alors faire commit et faire (git push) pour que le dépôt distant ait un historique.
...
1. Localement, configurez `git` avec votre nom (`user.name`) et votre email (`user.email`) (cf. [Personnalisation de Git](https://git-scm.com/book/fr/v2/Personnalisation-de-Git-Configuration-de-Git));
    ```bash
    git config --global user.name "AsmaaBaiche"
git config --global user.email "asmaa.uvsq@gmail.com"
    ```
...
> J'utilise Windows 10/11, avec Git Bash comme émulateur de terminal bash (basé sur MinGW), qui permet d'exécuter des commandes Unix/Linux sous Windows.
